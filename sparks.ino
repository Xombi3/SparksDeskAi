...
int  petsToday = 0;
int  petsTotal = 0;
char robotName[EEPROM_NAME_LEN] = "SPARKS"; // hardcoded  this is Sparks
...
#define SAYING_COUNT 44
#define SAYING_TIME_LATE   0  // index of time-of-day sayings
#define SAYING_TIME_MORN   1
#define SAYING_TIME_MID    2
#define SAYING_TIME_EVE    3

// Bubble state
bool   bubbleActive   = false;
int    bubbleIdx      = -1;
unsigned long bubbleStartMs  = 0;
unsigned long bubbleShowMs   = 7000; // how long it stays up
...
bool   chattingLook          = false; // eyes look left while bubble up
...
void renderEyes(){
  ...
  // Draw hearts inside sprite so they get auto-cleared
  drawHeartsInSprite();
  drawLoveHearts(lx,ly,rx,ry,rw,rh);
...
void setExpr(Expr e){
  ...
  if(!iAmA){
    ...
    switch(e){
      ...
      case LOVESTRUCK:  setRGB(255,0,120); eyeColour=0xF81F; break; // hot pink
    }
  }
}
...
void triggerMilestone(int idx){
  ...
  switch(idx){
    ...
    case 1: // 50 pets  excited expression + stars burst from both eyes
      ...
      spawnStars(BASE_ELX, SPR_Y+ECY);
      spawnStars(BASE_ERX, SPR_Y+ECY);
    ...
    case 3: // 500 pets  full chaos: confetti + stars + hearts + dizzy + RGB white flash
      pushMood(EXCITED); setExpr(EXCITED);
      spawnConfetti();
      confettiActive = true;
...
void friendTick(){
  ...
  if(!udpStarted){
    ...
    udpStarted = true;
    deviceId = 1; iAmA = true;
    Serial.println("I am SPARKS (device A)");
  }
...
void sprHeartEye(int cx, int cy, int r, uint16_t col){
  // Filled heart centred at cx,cy with radius r, drawn into sprite
  if(r < 3) return;
  cx = constrain(cx, r+2, SPR_W-r-2);
  cy = constrain(cy, r+2, SPR_H-r-2);
  spr.fillCircle(cx - r/2, cy - r/4, r/2, col);
  ...
  // Glisten dot
  spr.fillCircle(cx + r/3, cy - r/3, max(1,(int)(r*0.18f)), 0xFFFF);
}

void updateLovestruck(float dt){
  if(curExpr == LOVESTRUCK){
    if(!lovestruck){ lovestruck=true; loveHeartL=0; loveHeartR=0; lovePulse=0; }
    lovePulse += dt * 3.f;
    float pulse = 1.f + sinf(lovePulse) * 0.12f;
    loveHeartL = elf(loveHeartL, pulse, dt*3.f);
    loveHeartR = elf(loveHeartR, pulse, dt*2.5f);
  } else {
    lovestruck = false;
    loveHeartL = elf(loveHeartL, 0, dt*5.f);
    loveHeartR = elf(loveHeartR, 0, dt*5.f);
  }
}
...
void bubbleErase(){
  // Wipe the bubble area  top-right of screen, above sprite
...
void bubbleDraw(){
  // Bubble sits in top area above the sprite (y=0 to y=SPR_Y-2)
  // Text area: leave 4px padding inside bubble
  const int BX = 8;       // bubble left x
  const int BY = 4;       // bubble top y
  const int BW = SW - 16; // bubble width
  const int BH = SPR_Y - 14; // bubble height (leaves room for tail)
  const int BRAD = 8;     // corner radius
  uint16_t bubCol  = 0x2104; // very dark grey (near black)
  uint16_t textCol = 0xFFFF;
  uint16_t bordCol = eyeColour;

  // Clear area first
  tft.fillRect(0, 0, SW, SPR_Y-1, BLK);

  // Filled bubble
  tft.fillRoundRect(BX, BY, BW, BH, BRAD, bubCol);
  // Border
  tft.drawRoundRect(BX,   BY,   BW,   BH,   BRAD, bordCol);
  tft.drawRoundRect(BX+1, BY+1, BW-2, BH-2, BRAD, bordCol);

  // Tail  triangle pointing down toward left eye
  // Tail tip = approx screen x=90, y=SPR_Y-1 (just above sprite, near left eye)
  int tx = 90, ty = SPR_Y - 1;
  int tb = BY + BH; // tail base y
  // Draw filled triangle: base at bubble bottom-left area, tip at eye
  tft.fillTriangle(BX+10, tb, BX+28, tb, tx, ty, bubCol);
  tft.drawLine(BX+10, tb,   tx, ty, bordCol);
  tft.drawLine(BX+28, tb,   tx, ty, bordCol);

  // Text: split on newline, render each line
  tft.setTextSize(1);
  tft.setTextColor(textCol);
  int lineH = 10;
  int lines  = 0;
  // Count lines to vertically centre
  const char* p = bubbleText;
  int nLines = 1;
  while(*p){ if(*p=='\n') nLines++; p++; }
  int totalH = nLines * lineH;
  int startY = BY + (BH - totalH)/2;
  ...
  while(true){
    ...
    if(*p=='\n' || done){
      lineBuf[li]=' ';
      int lw = strlen(lineBuf)*6;
      tft.setCursor(BX + (BW-lw)/2, startY + row*lineH);
      tft.print(lineBuf);
      row++; li=0;
      if(done) break;
    } else {
      if(li<47) lineBuf[li++]=*p;
    }
    p++;
  }
...
void setup(){
  ...
  if(wifiOK){ udp.begin(FRIEND_PORT); udpStarted=true; }
  setExpr(timeExpr());
...
void loop(){
  ...
  if(ms-ldX>dpX){
    ...
    switch(curExpr){
      case ANGRY:    dpX=random(1200,2200);dTX=(float)(random(2)?1:-1)*random(25,55); break;
      case SLEEPY:   dpX=random(9000,15000);dTX=(float)random(-10,10); break;
      case HAPPY:    dpX=random(2000,4000);dTX=lf(dTX,(float)random(-40,40),0.5f); break;
      case SURPRISED:dpX=random(1500,3000);dTX=(float)(random(2)?1:-1)*random(20,50); break;
      case CONFUSED: dpX=random(2000,4000);dTX=(float)(random(2)?1:-1)*random(15,40); break;
      case EXCITED:  dpX=random(800,2000); dTX=(float)(random(2)?1:-1)*random(20,50); break;
      default:{dpX=random(4000,8000);int r=random(10);
        float rng=(r<5)?random(18,28):(r<8)?random(32,48):random(44,60);
  ...
  if(ms-ldY>dpY){
    ...
    switch(curExpr){
      ...
      case SAD:      dpY=random(4000,8000); dTY=(float)random(12,30); break;
      case HAPPY:    dpY=random(1500,3500); dTY=(float)random(-28,32)*attentionBoost; break;
      case ANGRY:    dpY=random(1200,2500); dTY=(float)(random(2)?1:-1)*random(14,30); break;
      case SURPRISED:dpY=random(1200,2500); dTY=(float)(random(2)?1:-1)*random(18,34); break;
      case CONFUSED: dpY=random(2000,5000); dTY=(float)(random(2)?1:-1)*random(10,28); break;
      case EXCITED:  dpY=random(800,2000);  dTY=(float)(random(2)?1:-1)*random(16,32); break;
      case EMBARRASSED:dpY=random(3000,6000);dTY=(float)random(14,30); break;
      default:{dpY=random(3500,7000);int r=random(10);
        float rng=(r<5)?random(16,24):(r<8)?random(24,32):random(28,36);
  ...
  driftX=cf(driftX,-60.f,60.f);
  driftY=cf(driftY,-10.f,36.f);
  // If drift target is out of bounds, pull it back
  dTX=cf(dTX,-60.f,60.f);
  ...
  if(ms-lastJitter>75){
    lastJitter=ms;
    microJitterX=((float)(random(60)-30))/100.f; // -0.3 to +0.3
    microJitterY=((float)(random(60)-30))/100.f;
  }
  ...
  if(curPage==PAGE_EYES && embarrassBlush>0.05f){
    ...
    tft.fillEllipse(BASE_ELX,  SPR_Y+ECY+BASE_RH+8, (int)(18*embarrassBlush), (int)(6*embarrassBlush), bc);
    tft.fillEllipse(BASE_ERX,  SPR_Y+ECY+BASE_RH+8, (int)(18*embarrassBlush), (int)(6*embarrassBlush), bc);
  } else {
    // Erase blush only once as it fades out completely
    static float lastBlush = 0;
    if(lastBlush > 0.05f && embarrassBlush <= 0.05f){
      tft.fillEllipse(BASE_ELX, SPR_Y+ECY+BASE_RH+8, 22, 9, BLK);
      tft.fillEllipse(BASE_ERX, SPR_Y+ECY+BASE_RH+8, 22, 9, BLK);
    }
